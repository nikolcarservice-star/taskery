...
export function ClientProfileMenu({
  ...
  const mainLinks = useMemo<MenuLinkItem[]>(
    () => [
      { label: menu.myCabinet, href: l("/client") },
      { label: menu.projects, href: l("/client/projects") },
      { label: menu.finances, href: l("/client/finances") },
    ],
    [l, menu.finances, menu.myCabinet, menu.projects],
  );

  const updateLinks: MenuLinkItem[] = [{ label: menu.whatsNew, href: l("/faq") }];

  const settingsLinks: MenuLinkItem[] = [
    { label: menu.personal, href: l("/client/personal") },
    { label: menu.settings, href: l("/client/settings") },
  ];
          ...
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-zinc-900">
                {name ?? menu.defaultNames.client}
              </p>
              <UserRoleLabel roleLabel={menu.roles.client} isAdmin={showAdminPanel} />
            </div>
          </div>
